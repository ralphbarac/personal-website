defmodule Website.Repo.Migrations.AddDatabaseConstraintsAndIndexes do
  ...
  def up do
    ...
    create constraint(:photos, :priority_score_range, check: "priority_score >= 1 AND priority_score <= 10")
    
    # Add check constraints for visual_weight enum values
    create constraint(:photos, :visual_weight_enum, check: "visual_weight IN ('light', 'medium', 'heavy')")
    
    # Add check constraints for focal point ranges (0.0-1.0)
    create constraint(:photos, :focal_point_x_range, check: "focal_point_x >= 0.0 AND focal_point_x <= 1.0")
    create constraint(:photos, :focal_point_y_range, check: "focal_point_y >= 0.0 AND focal_point_y <= 1.0")
    ...
    alter table(:photos) do
      modify :photo_category_id, references(:photo_categories, on_delete: :restrict, on_update: :update_all)
    end
    
    alter table(:projects) do
      modify :project_status_id, references(:project_statuses, on_delete: :restrict, on_update: :update_all)
    end
  end
