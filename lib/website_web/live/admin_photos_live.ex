      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-8">
          ...
          <div :if={@category_changeset} class="border border-slate-300 rounded-lg p-4 mb-4 bg-slate-50">
            ...
            <.form :let={f} for={@category_changeset} phx-submit="create_category" phx-change="validate_category">
              ...
              <div class="mt-4 flex gap-2">
                ...
                <button type="button" phx-click="cancel_category" class="bg-slate-600 text-white px-4 py-2 rounded-lg hover:bg-slate-700">
                  Cancel
                </button>
              </div>
            </.form>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div class="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center">
                  <.live_file_input upload={@uploads.image} class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-slate-50 file:text-slate-700 hover:file:bg-slate-100" />
                </div>
              ...
              <div>
                ...
                <.input field={f[:photo_category_id]} type="select" options={[{"Select a category...", nil} | Enum.map(@categories, &{&1.name, &1.id})]} />
              </div>
            </div>
        ...
        <div :if={@editing_photo} class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-8">
          <h2 class="text-xl font-semibold text-slate-900 mb-4">Edit Photo</h2>
          
          <.form :let={f} for={@changeset} phx-submit="update">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <img src={@editing_photo.image_path} alt={@editing_photo.description} class="w-full h-48 object-cover rounded-lg" />
              </div>
            ...
            <div class="mt-6 flex justify-end gap-2">
              <button type="button" phx-click="cancel_edit" class="bg-slate-600 text-white px-6 py-2 rounded-lg hover:bg-slate-700">
                Cancel
              </button>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
          <h2 class="text-xl font-semibold text-slate-900 mb-6">Existing Photos (<%= length(@photos) %>)</h2>
          
          <div :if={length(@photos) == 0} class="text-center py-12 text-slate-500">
            No photos uploaded yet. Upload your first photo above!
          </div>
          
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div :for={photo <- @photos} class="border border-slate-200 rounded-lg overflow-hidden">
              ...
              <div class="p-4">
                ...
                <p class="text-xs text-slate-500 mb-3">Category: <%= if photo.photo_category, do: photo.photo_category.name, else: "Uncategorized" %></p>
